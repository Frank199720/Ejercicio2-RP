            ...
            try
            {
                ...
                await Task.Run(() => {
                    ...
                    var upgradeEngineBuilder = DeployChanges.To.SqlDatabase(connectionString, null)
                                                            ...
                                                            .WithScriptsEmbeddedInAssembly(Assembly.GetExecutingAssembly(), x => x.StartsWith($"DatabaseHandler.SQLScripts.BeforeDeployment."), new SqlScriptOptions { ScriptType = ScriptType.RunOnce, RunGroupOrder = 0 })
                                                            // Main Deployment scripts: se ejectan solo una vez y corren en el segundo grupo.
                                                            .WithScriptsEmbeddedInAssembly(Assembly.GetExecutingAssembly(), x => x.StartsWith($"DatabaseHandler.SQLScripts.Deployment."), new SqlScriptOptions { ScriptType = ScriptType.RunOnce, RunGroupOrder = 1 })
                                                            // Post deployment scripts: siempre se ejecutan estos scripts después de que todo se haya implementado.
                                                            .WithScriptsEmbeddedInAssembly(Assembly.GetExecutingAssembly(), x => x.StartsWith($"DatabaseHandler.SQLScripts.PostDeployment."), new SqlScriptOptions { ScriptType = ScriptType.RunAlways, RunGroupOrder = 2 })
